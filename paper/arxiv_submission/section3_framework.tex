...
\caption{FolT-MCMC pipeline.
  ...
  certifies via the quotient-metric LCNF framework.}
\label{fig:schematic}
\end{figure}

FolT-MCMC combines folding, transport, sampling, and certification
...
\quad Solve the covering condition~\eqref{eq:covering_cond_F}
  for $\varepsilon_F^*$. \\
\quad Report the quantile-core certified lower bound
...
\textit{Output:} certified posterior samples on~$D$;
  quantile-core certificate~$\mathrm{QC}\,\underline{\gamma}_F$.
}}
\end{center}
...
The resulting quantile-core certificate is developed in
